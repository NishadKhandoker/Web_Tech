...
$u = $_SESSION['user'] ?? [];
$profile = $profile ?? [];
$stats = $stats ?? ['internships_applied'=>0,'research_requests'=>0];
?>
...
<div style="display:grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap:12px; margin:12px 0;">
  <div class="card">
    <div class="muted">Applied Internships</div>
    <div style="font-size:28px; font-weight:800; margin-top:6px;"><?= (int)$stats['internships_applied'] ?></div>
  </div>
  <div class="card">
    <div class="muted">Research Requests</div>
    <div style="font-size:28px; font-weight:800; margin-top:6px;"><?= (int)$stats['research_requests'] ?></div>
  </div>
</div>
...
<div class="card" style="margin-top:12px;">
  <div style="display:flex; justify-content:space-between; align-items:center; gap:12px;">
    ...
    <a class="btn" href="index.php?page=student.update">Update Information</a>
  </div>
  ...
  <div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px; margin-top:14px;">
    ...
    <div>
      ...
      <div><?= htmlspecialchars($profile['major'] ?? '') ?></div>
    </div>
    <div>
      <div class="muted">CGPA</div>
      <div><?= htmlspecialchars((string)($profile['cgpa'] ?? '')) ?></div>
    </div>

    <div>
      <div class="muted">Credits Completed</div>
      <div><?= htmlspecialchars((string)($profile['credits_completed'] ?? '')) ?></div>
    </div>
    <div>
