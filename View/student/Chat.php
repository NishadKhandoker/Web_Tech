...
function renderMessageRow(array $m, int $meId): string {
  ...
  $bubbleStyle = $isMine
    ? 'max-width:70%; background:#dbeafe; color:#1e3a8a; border:1px solid #93c5fd; padding:10px 12px; border-radius:14px;'
...
<div class="card" style="margin-bottom:12px;">
  <div style="display:flex; justify-content:space-between; align-items:center; gap:12px; flex-wrap:wrap;">
    ...
    <a class="btn" style="background:#6b7280;" href="index.php?page=student.messages">Back</a>
  </div>
</div>
...
async function poll(){
  try{
    ...
    const res = await postJSON("index.php?page=student.chat.fetch.ajax", {
      group_id: GROUP_ID,
      after_id: afterId
    });
    if(!res.ok) return;
...
async function sendMsg(){
  ...
  const out = await postJSON("index.php?page=student.chat.send.ajax", {
    group_id: GROUP_ID,
    message: msg
  });
